function[]=select_kernel_step(lambda,eta,p,name_kernel,file,name_data)
load(name_data);
get_div=read_diversity_parameter(covariance,'parameter_diversity');
q=p/(p-1);
...
second_array=get_diversity_batch(get_div,kernel_sel,kernel_left);
temp_first_array=(1/(8*lambda))*first_array;
% obj=zero_array-temp_first_array;
final_array=temp_first_array+eta*second_array;
final=[kernel_left temp_first_array eta*second_array final_array];
